clear;
% Testing various metric on adversial stiuation
...
for i =1:10
    ...
    xT = xT(xTi,:);
    yT = yT(xTi);
    %%%%%Adversarial  examples by changing label ~1=0, ~0=1
    fperm= randperm(size(xt,2));
    t=xt(:,fperm(1:47));
    t1 = ~t;
    xt2 = xt;
    xt2(:,fperm(1:47))=t1;
    %%%%% end adversial
    Res1(i,:) = RSS(xT,yT,xt2,yt); %RSS
    Res2(i,:) = IR(xT,yT,xt2,yt); %IRS nearest center
    Res3(i,:) = ERisk(xT,yT,xt2,yt); % ERS
    Res4(i,:) = NRS(xT,yT,xt2,yt); % NRS nearest neighebur with hamming distance
